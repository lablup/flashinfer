...
The SM12x B12x MoE kernels (`b12x_moe_<arch>_cute_dsl/`) also persist through
this cache. Their specializations are device-specific (SM count participates),
so pre-bake caches on the serving GPU model with the warmup tool:
`python -m flashinfer.fused_moe.cute_dsl.blackwell_sm12x.warmup
--config <model_dir> --tp-size N [--workers K]` — it drives the real wrapper
dispatch so cache keys match a vLLM deployment by construction, and it handles
per-layer expert counts (`n_routed_experts_per_layer`).
...
| `FLASHINFER_B12X_PAD_IN_PLACE` | `1` | `flashinfer/fused_moe/cute_dsl/b12x_moe.py` | `0` disables `B12xMoEWrapper.run`'s in-place weight rebind when padding non-128-aligned intermediate sizes (e.g. Solar-Open2 TP4: 1280/4 = 320). With the rebind, the caller's weight tensors are repointed at the padded storage and the unpadded originals are freed; disabled, original + padded copies both stay resident (~2× MoE weight memory, severely shrinking vLLM's KV cache). |
